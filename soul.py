...
def handle_message(message):
    ...
    if message.text == "Instant Plan 🧡":
        ...
        bot.reply_to(message, "*Instant++ Plan selected. Please provide the target IP, port, and duration (in seconds).*", parse_mode='Markdown')
        bot.register_next_step_handler(message, process_attack_command)  # Wait for parameters
    elif message.text == "Canary Download✔️":
        bot.send_message(message.chat.id, "*Download link: https://t.me/Danger_hack_ddos/19*", parse_mode='Markdown')
    elif message.text == "My Account🏦":
        user_data = users_collection.find_one({"user_id": message.from_user.id})
        if user_data:
            days_approved = (datetime.fromisoformat(user_data['valid_until']) - datetime.now()).days
            bot.send_message(message.chat.id, f"*Your Telegram User ID: {message.from_user.id}\nDays Approved: {days_approved}*", parse_mode='Markdown')
        else:
            bot.send_message(message.chat.id, "*No account info found.*", parse_mode='Markdown')
        ...
        bot.send_message(message.chat.id, "*I don't understand that. Please choose an option from the menu.*", parse_mode='Markdown')
