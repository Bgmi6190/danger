...
def handle_message(message):
    ...
    if message.text == "Instant Plan 🧡":
        ...
        if user_data:
            ...
            plan = "Instant Plan 🧡" if user_data.get('plan') == 1 else "Instant++ Plan 💥" if user_data.get('plan') == 2 else "N/A"
            valid_until = user_data.get('valid_until', 'N/A')
            current_time = datetime.now().astimezone(tz=timedelta(hours=5, minutes=30)).strftime('%Y-%m-%d %H:%M IST')
