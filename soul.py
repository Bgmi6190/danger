...
def stop_attack(message):
    ...
    if user_id in running_processes:
        ...
        process.terminate()  # Stop the ongoing attack
        del running_processes[user_id]
...
def handle_message(message):
    ...
    if message.text == "Instant Plan 🧡":
        ...
        # Allow access only for approved users
        if check_user_approval(user_id):
            bot.reply_to(message, "*Instant++ Plan selected*", parse_mode='Markdown')
            attack_command(message)  # Proceed with attack command
        else:
            bot.reply_to(message, "*You are not approved to access the Instant++ Plan 💥.*", parse_mode='Markdown')
        ...
        if user_data:
            ...
            current_time = datetime.now(pytz.timezone('Asia/Kolkata')).strftime("%H:%M:%S")  # Current time in IST
            bot.send_message(message.chat.id, f"*Your Account Details:*\n\nUser ID: `{user_id}`\nPlan: {plan}\nValid Until: {valid_until}\nAccess Count: {access_count}\nCurrent Time: {current_time} (IST)", parse_mode='Markdown')
        else:
            bot.send_message(message.chat.id, "*No account details found.*", parse_mode='Markdown')
    elif message.text == "Help❓":
        bot.send_message(message.chat.id, "*Please contact @jks6190 for help.*", parse_mode='Markdown')
    elif message.text == "Contact admin✔️":
        bot.send_message(message.chat.id, "*You can reach the admin at: @jks6190*", parse_mode='Markdown')
